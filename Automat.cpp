...
int main()
{
    ...
    TransitionFunction<char> a(initialState, firstState, 'a');
    TransitionFunction<char> b(initialState, initialState, 'b');

    TransitionFunction<char> c(firstState, firstState, 'a');
    TransitionFunction<char> d(firstState, secondState, 'b');

    TransitionFunction<char> e(secondState, finalState, 'a');
    TransitionFunction<char> f(secondState, initialState, 'b');

    TransitionFunction<char> g(finalState, firstState, 'a');
    TransitionFunction<char> h(finalState, secondState, 'b');

    Automata<char> fsm({ initialState, firstState, secondState, finalState }, { a,b,c,d,e,f,g,h }, { 'a','b' }, initialState);

    std::string parsedString = "bbabbbbabab";
    std::vector<char> s(parsedString.begin(), parsedString.end());
    cout << parsedString << endl;
    cout << fsm.Read(s) << endl;
    return 0;
}